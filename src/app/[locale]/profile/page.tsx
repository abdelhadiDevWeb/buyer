...
const useTranslation = () => ({ t: (k: string) => k })
...
function ProfilePage() {
  ...
  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 5 * 1024 * 1024) {
      enqueueSnackbar(t('profile.fileSizeTooLarge'), { variant: 'error' });
      return;
    }

    const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'];
    if (!allowedTypes.includes(file.type)) {
      enqueueSnackbar(t('profile.invalidImageFormat'), { variant: 'error' });
      return;
    }
    ...
    try {
      ...
      if (response) {
        if (response.user) {
          ...
          const updatedUser = {
            ...
            photoURL: (response.user as any)?.photoURL || (response as any).avatarUrl,
      ...
      const errorMessage = error.response?.data?.message || error.message || t('profile.failedToUploadAvatar');
      enqueueSnackbar(errorMessage, { variant: "error" });
    } finally {
    ...
    return (
      <div className="profile-login-required">
        <div className="login-prompt">
          <h2>{t('profile.authenticationRequired')}</h2>
          <p>{t('profile.loginRequired')}</p>
          <button onClick={() => router.push('/auth/login')}>{t('profile.goToLogin')}</button>
        </div>
      </div>
    );
  }
                        ...
                        <div className="rating-stars-modern">
                          {auth.user?.rate && auth.user.rate > 0 ? (
                            ...
                            >
                              ...
                              <span>{t("profile.noRatingAvailable")}</span>
                            </motion.div>
                          )}
                        </div>
                        ...
                        <div className="section-header">
                          <div className="header-content">
                            ...
                            <div className="header-text">
                              ...
                              <p>{t("profile.manageProfileDescription")}</p>
                            </div>
                          </div>
                          <motion.button
                            ...
                            {[
                              ...
                              >
                                ...
                                <input
                                  ...
                                  placeholder={
                                    field.name === 'currentPassword' ? t('profile.enterCurrentPassword') :
                                    field.name === 'newPassword' ? t('profile.enterNewPassword') :
                                    field.name === 'confirmPassword' ? t('profile.enterConfirmPassword') :
                                    `Entrez ${field.label}`
                                  }
                                />
                              </motion.div>
                        ...
                        >
                          {[
                            { 
                              ...
                              title: t("profile.emailNotificationsTitle"), 
                              desc: t("profile.emailNotifications"),
                              color: "primary",
                              gradient: "linear-gradient(135deg, #3b82f6 0%, #1e40af 100%)"
                            },
                            { 
                              icon: "bi-bell-fill", 
                              title: t("profile.newAuctionAlertsTitle"), 
                              desc: t("profile.newAuctionAlerts"),
                              color: "success", 
                              gradient: "linear-gradient(135deg, #10b981 0%, #059669 100%)"
                            },
                            { 
                              icon: "bi-heart-pulse", 
                              title: t("profile.bidUpdatesTitle"), 
                              desc: t("profile.bidUpdates"),
                              color: "warning",
