  ...
  const fetchUserDetails = useCallback(async () => {
    try {
      ...
      if (userData) {
        ...
        const isRecommended = recProfIds.includes(actualUserData._id) || recResellerIds.includes(actualUserData._id);

        const transformedUser: User = {
          _id: actualUserData._id,
          firstName: actualUserData.firstName || 'Unknown',
          lastName: actualUserData.lastName || '',
          email: actualUserData.email || 'No email',
          avatar: actualUserData.photoURL || actualUserData.avatar?.url || actualUserData.avatar,
          photoURL: actualUserData.photoURL,
          type: actualUserData.type || actualUserData.accountType || USER_TYPE.CLIENT,
          role: actualUserData.role,
          rating: typeof actualUserData.rate === 'number' ? actualUserData.rate : (actualUserData.rating || 0),
          joinDate: actualUserData.createdAt || new Date().toISOString(),
          totalBids: actualUserData.totalBids || 0,
          winningBids: actualUserData.winningBids || 0,
          totalAuctions: actualUserData.totalAuctions || 0,
          completedAuctions: actualUserData.completedAuctions || 0,
          phone: actualUserData.phone,
          location: actualUserData.location,
          description: actualUserData.description,
          verificationStatus: actualUserData.verificationStatus || (actualUserData.isVerified ? 'verified' : 'unverified'),
          isRecommended: Boolean(actualUserData.isRecommended || isRecommended),
          history: actualUserData.history || []
        };
